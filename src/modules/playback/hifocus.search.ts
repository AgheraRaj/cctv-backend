...
export const searchHifocusRecordings = async (
  ...
  try {
    ...
    logger.error(
      `Hifocus: cannot connect to ${ip}:${httpPort}: ${String(err)}`,
    );
  ...
  if (summary) {
    summaryFrom =
      summary.dataFrom instanceof Date
        ? summary.dataFrom
        : new Date(summary.dataFrom);
    summaryUntil =
      summary.dataUntil instanceof Date
        ? summary.dataUntil
        : new Date(summary.dataUntil);
  ...
  logger.info(
    `Hifocus ${ip}: ${allRecordings.length} total recording token(s)`,
  );
  ...
  for (const { token, info } of infos) {
    ...
    if (!recStart || !recEnd) {
      logger.warn(
        `Hifocus ${ip}: no time range for token "${token}" — skipping`,
      );
      continue;
    }
  ...
  logger.info(
    `Hifocus ${ip} ch${channel}: returning ${segments.length} segment(s)`,
  );
  return segments;
};